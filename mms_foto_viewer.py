...
/***************************************************************************
  ...
  QGIS plugin displaying photos from Mobile Mapping System
  Copyright: Kyrylo Zharkovskyi 2022
/***************************************************************************
 *
...
SCRIPT_TITLE = 'MMS Foto Viewer'
SCRIPT_NAME = 'mms_foto_viewer'
SCRIPT_VERSION = '2.0'
GENERAL_INFO = u"""
Plugin for viewing foto from Mobile Mapping System

© Kyrylo Zharkovskyi 2022
library GitHub road_inspection_viewer is used
license: GPL v. 2
"""
...
class mms_foto_viewer(QtWidgets.QMainWindow):
    ...
    def  __init__(self, iface):
        super(mms_foto_viewer, self).__init__()
        self.iface = iface
        ...
        self.icon = QtGui.QIcon(os.path.join(self.base_path, 'img', 'mfv_ico.png'))
        self.start_image = QtGui.QImage(os.path.join(self.base_path, 'img', 'start_image.png'))
        self.no_file_image = QtGui.QImage(os.path.join(self.base_path, 'img', 'no_file_image.png'))
        # KZ self.path_to_photos = r'C:\Users\donat\Desktop\3sem\SF_kosz\Przyklad\Przyklad'
        self.delay = 700 # ms
        self.raw_image = None
...
##############################################################################      
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_D:
            self.Forward_handleButton()
            
        if e.key() == Qt.Key_A:
            self.Back_handleButton()
        e.accept()
        QtWidgets.QMainWindow.keyPressEvent(self, e)
##############################################################################
...
##############################################################################
    ...
    def initGui(self):
        ...
        self.action.setObjectName('mms_foto_viewer_Action')
        self.action.triggered.connect(self.run)
        self.action.triggered.connect(self.Path_handleButton)
        self.iface.addToolBarIcon(self.action)
        uic.loadUi(os.path.join(self.base_path, 'ui', 'mms_foto_viewer.ui'), self)
        ...
        self.zts.clicked.connect(self.zts_def)
        
        #self.follow = QCheckBox("follow")
        self.follow.setChecked(False)
        #self.follow.stateChanged.connect(self.follower)
        # enebled buttons:
    ...
    def unload(self):
        ...
        self.zts.clicked.disconnect(self.zts_def)
...
##########################################        
    # def keyPressEvent(self, e):
    #     super(mms_foto_viewer, self).keyPressEvent(e)
...
##########################################
    ...
    def Path_handleButton(self):
        if self.get_first_selected_point_folderPath() is None:
            photos_folder = None
        ...
        # else:
        #     photos_folder = self.path_to_photos
        photos_folder = self.get_first_selected_point_folderPath()
        if  photos_folder != None and os.path.exists(photos_folder):
    ...
    def Play_back_handleButton(self):
        ...
        self.p_thread = play(self, self.delay, False)
    ...
    def Back_handleButton(self):
        if self.Back_pushButton.isEnabled():
            ...
            if self.follow.isChecked() == True:
                layer = self.iface.activeLayer()
                box = layer.boundingBoxOfSelected()
                self.iface.mapCanvas().setExtent(box)
                self.iface.mapCanvas().zoomScale(300)
    ...
    def Play_forward_handleButton(self):
        ...
        self.p_thread = play(self, self.delay, True)
        self.p_thread.start_run()
    ...
    def CRE(self, event):
        ...
        p = self.iface.toMapCoordinates(event.pos())
        w = self.iface.mapUnitsPerPixel() * 10
        try:
...
##############################################################################
    def zts_def(self):
        ...
        box = layer.boundingBoxOfSelected()
        self.iface.mapCanvas().setExtent(box)
        self.iface.mapCanvas().zoomScale(200)
        self.iface.mapCanvas().refresh()
...
##############################################################################
    ...
    def get_first_selected_point_folderPath(self):
        layer = self.iface.activeLayer()
        if layer:
            selection = layer.selectedFeatures()
            if selection and len(selection) > 0:
                return str(os.path.dirname(os.path.dirname(os.path.dirname(selection[0]['path']))))
